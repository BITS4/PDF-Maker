...
internal suspend fun docxToPdf(
    ...
    val providerInput = context.contentResolver.openInputStream(uri)
    ...
    try {
        ...
        extractDocxConversionArchive(stagedDocx, workingDirectory).use { archive ->
            operationContext.ensureActive()
            onProgress(30, "Parsing content…")
            val relationships = archive.relationshipsXml
                ?.let(::parseConversionRelationships)
                .orEmpty()
            val blocks = parseConversionDocument(archive.documentXml, relationships)
            operationContext.ensureActive()
